...
class HomeFragment : Fragment() {
    ...
    override fun onStart() {
        ...
        if (filterBy == "time") {
            setups.listSetupsByTime(order)
                .addOnSuccessListener { documents ->
                    ...
                    no_setup?.setText("Terjadi kesalahan, tapi jangan khawatir, itu bukan salah Anda.\n" +
                            "Coba lagi nanti.")
                }
        } else {
        ...
        setup_filter.setOnClickListener {
            ...
            mDialogView.time_inc.setOnClickListener {
                mAlertDialog.dismiss()
                val intent = Intent(context, MenuActivity::class.java)
                intent.putExtra("filterBy", "time")
                intent.putExtra("order", "inc")
                startActivity(intent);
            }
            mDialogView.price_inc.setOnClickListener {
                mAlertDialog.dismiss()
                val intent = Intent(context, MenuActivity::class.java)
        ...
        fab.setOnClickListener {
            ...
            mDialogView.submit_dialog.setOnClickListener {
                ...
                if (dialogText.isEmpty()) {
                    mDialogView.dialogEditText.setError("Kolom tidak boleh kosong")
                } else {
                    ...
                    setups.listSetupsByTime(order).addOnSuccessListener { documents ->
                        ...
                        for (document in documents) {
                            if (document.data.get("name").toString().equals(dialogText)) {
                                mDialogView.dialogEditText.setError("Nama telah digunakan")
                                equal = true
                            }
                            i++
                        }
                        ...
                        if (!equal) {
                            ...
                            if (setup != null) {
                                setups.add(setup).addOnSuccessListener {
                                    Toast.makeText(context, "Ditambahkan.", Toast.LENGTH_LONG)
                                        .show()
    ...
    public fun setSetups(documents: QuerySnapshot?) {
        ...
        if (i === 0) {
            no_setup.visibility = View.VISIBLE
            recyclerView.visibility = View.GONE
            no_setup.setText("Setup masih kosong. \n Klik + untuk menambahkan setup Anda.")
        } else {
