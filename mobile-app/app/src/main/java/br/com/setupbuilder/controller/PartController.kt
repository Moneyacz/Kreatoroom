...
class PartController {
    ...
    public fun listPartsByTerm(term: String, order: String): Task<QuerySnapshot> {
        if(order == "desc")
            return setupFirebase.whereEqualTo("product", term).orderBy("price", Query.Direction.DESCENDING).get()
        else
            return setupFirebase.whereEqualTo("product", term).orderBy("price", Query.Direction.ASCENDING).get()
    }
